...
const stripe = require("stripe")("sk_test_L5zyOFjRYX7s2JVbBHYClsbP00ojt9TXsA");
...
app.post("/insertDealerInformation", (request, response) => {
	...
	.then(function(hashedPassword){
		...
		console.log('hashed password', hashedPassword);
			...
			var link = "https://beta.buscadordeauto.com/verification?id="+rand;
...
app.post('/verify',function(req,response){
	...
	if(req.body.id==rand)
	{
		...
		// res.end("<h1>Email "+mailOptions2.to+" is been Successfully verified");
	}
	else
	{
	...
	{
		console.log("Request is from unknown resource");
		...
		if(!user[0]){
			...
			const mailOptionsforgotPasswor = {  
                ...
                text: 'You are receiving this because you have requested the reset of the password for your account.\n\n' +  
                    ...
                    'https://beta.buscadordeauto.com/Cambia%20la%20contrase%C3%B1a?id=' + userid + '\n\n' +  
...
app.get("/getAllDealersDataDealerManagement", (request, response) => {
	console.log('urlllll', process.env.FORGOT_PASSWORD);
	collection.find({}).toArray(function(err, res) {
		if (err){
			console.log('errors while fetching all data');
			return response.status(400).send();
...
app.get("/getAllDealersHomenetInventory", (request, response) => {
	dealersInventory.find({}).toArray(function(err, res) {
		if (err){
			console.log('errors while fetching all data');
			logger.error('Its error log', err);
			return response.status(400).send();
		};
		logger.info('Inventory data fetched');
		return response.send(res);
	  });
});
...
app.post("/getHomenetInventoryByDealerId", (request, response) => {
	...
	// process.exit();
	var dealerId = request.body.dealerId.replace("_", "-");
	dealersInventories.find({"vehicle.vehicleId.dealerId": dealerId }).toArray(function(err, res) {
		if (err){
			return response.status(400).send();
		};
				...
				if (err){
					...
					const mailOptionsSuspendAccount = {  
						...
						text: 'Your account has been suspended for'  +  user[0].description  +  'non-payment.\n\n' +  
							...
							'For Payment:' + 'https://beta.buscadordeauto.com/suspendPayment?id=' + userid + '\n\n'   
					};  
